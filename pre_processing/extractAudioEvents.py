...
def extract_audio_events(file_name, make_events_wav_files_logic=False):
    ...
    # Read audio file and set signal, sample frequency, and time vectors
    ...
    #Set desired time per window and overlap fraction
    t_wind = 0.05
    n_wind = int(np.ceil(t_wind * fs))
    ovlp_frac = 0.75
    ovlp_num = int(np.ceil(n_wind * ovlp_frac))
    #Using keiser window
    window = np.kaiser(n_wind, 5)  
    #Perform stft
    ...
    #collect all times and frequencies corresponding to powers above threshold
    t_select = TMat[select_mat]
    f_select = FMat[select_mat].reshape(-1,1)
    channel_select = np.where(select_mat)[1].reshape(-1,1)
    #array of unique times, and original array, indexed by instances of unique times
    t_select_unique, ic = np.unique(t_select, return_inverse=True)

    #Take unique times where events were detected, and cluster
...
file_name = r"C:\Users\jeffu\Documents\Recordings\05_20_2024\2024-05-16_10_59_16.wav"

y, fs = sf.read(file_name)

y = y[5*fs:,0].reshape(-1,1)
f_nyquist = fs / 2
num_channels = 1
num_samps = y.shape[0]
t = np.arange(num_samps) / fs
...
T_replicated_1 = np.repeat(T_transposed, num_freqs, axis=0)
